...
if (isset($_POST['usuario_cedula']) || isset($_POST['fecha_inicio']) || isset($_POST['fecha_final'])) {
...
if ($usuario_cedula == "" || $fecha_inicio == "" || $fecha_fin == "") {
    ?>
    <!DOCTYPE html>
    ...
    //Inicio de codigo para listar las usuarios
                ...
                if ($usuario_cedula == 0) { //Todas los usuarios
                    $rs_ciudadanos = $ins_reporte->obtener_solicitud_controlador("Solicitudes", $usuario_cedula, $fecha_inicio , $fecha_fin);
                     } else { //usuario especifico
                        $rs_ciudadanos = $ins_reporte->obtener_solicitud_controlador("SolicitudEspecifica", $usuario_cedula, $fecha_inicio , $fecha_fin);
                   }
    ...
    $pdf->Cell(0, 10, utf8_decode(strtoupper("Solicitudes realizadas desde el " . $fecha_inicio . " hasta el " . $fecha_fin)), 0, 0, 'L');
    $pdf->Ln(15);
    ...
    $pdf->Cell(125, 8, utf8_decode('MOTIVO'), 1, 0, 'C', false);
    ...
    if ($rs_ciudadanos->rowCount() > 0) {
        ...
        foreach ($rs_ciudadanos as $rows) {
            ...
            $nombre_ciudadano = substr($nombre_ciudadano, 0, 22) . "_";

         
            $nombre_motivo = $rows['solicitud_descripcion'];
            $nombre_motivo = substr($nombre_motivo, 0, 95) . ".";
            ...
            $pdf->Cell(125, 6, utf8_decode($nombre_motivo), 1, 0, 'L');
            $pdf->Cell(20, 6, utf8_decode($rows['solicitud_estado']), 1, 0, 'L', false);
            $pdf->Cell(30, 6, utf8_decode($rows['solicitud_inicio']), 1, 0, 'L', false);
            $pdf->Ln(6);
            $cont++;
            $numero+=1;
        }
